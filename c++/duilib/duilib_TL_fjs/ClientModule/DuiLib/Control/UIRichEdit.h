...
namespace DuiLib {
	...
	// -------------------------------------- Duilib名字空间中的全部(变量/函数)定义 --------------------------------------
	// 全局变量定义位置移动到.h文件中,方便其他文件访问 moify by Aries
	...
	public:
		...
		void SetAlign(DWORD iAlign);
		void SetPassword(bool bPassword = false);	// Add by Aries 2016/09/26
		bool IsReadOnly();
		void SetReadOnly(bool bReadOnly = true);
		bool IsReadOnlyShowCaret();
		void SetReadOnlyShowCaret(bool bReadOnlyShowCaret = true);
		void SetCanPaste(bool bCanPaste = true);
		bool IsCanPaste();
		bool GetWordWrap();
		...
		void Cut();
		void Paste();
		void GetClipboardText(CDuiString& pzText);
		void SetClipboardText(CDuiString& pzText);

		// 粘贴特殊数据 by Aries 2016/04/26
		BOOL CanPaste(UINT nFormat = 0);
		void PasteSpecial(UINT uClipFormat, DWORD dwAspect = DVASPECT_CONTENT, HMETAFILE hMF = 0);
		...
		CTxtWinHost* GetTxtWinHost();	//获取文本主机服务接口对象指针(by Aries)
		IRichEditOle* GetRichEditOle();

	protected:
		CTxtWinHost* m_pTwh;
		bool m_bInputMode;			//是否是输入模式(否则为提示语模式)
		...
		bool m_bRich;
		bool m_bReadOnly;
		bool m_bReadOnlyShowCaret;	//只读时,是否显示光标
		bool m_bCanPaste;			//是否允许粘贴
		bool m_bWordWrap;
		DWORD m_dwTextColor;
		int m_iFont;
		int m_iLimitText;
