...
namespace DuiLib
{
	...
	bool CTreeViewUI::OnDBClickItem( void* param )
	{
		...
		if(pMsg->sType == _T("itemdbclick"))
		{
			...
			if (pFolder->IsMouseEnabled())					//edit bt Aries 2016.6.21 当折叠按钮"接收鼠标消息"开关打开时，消息才能继续传传递
		...
		if(_TreeNode)
		{
			if(_TreeNode->GetCountChild() > 0)
			{
				...
				for(int nIndex = 0;nIndex < nCount;nIndex++)
				{
					...
					if (pItem->GetCountChild() && !pFolderButton->IsSelected())
					{
						pFolderButton->Selected(_Expanded);	//折叠根节点时,也顺带折叠对应子节点  Modify by Aries 2016/06/25
