...
namespace DuiLib {
	...
	bool CComboUI::SelectItem(int iIndex, bool bTakeFocus)
	{
		...
		if (iIndex == m_iCurSel)
		{
			if (m_pManager != NULL) m_pManager->SendNotify(this, DUI_MSGTYPE_ITEMSELECT, m_iCurSel, m_iCurSel);  // Add by Aries 2017.04.13  自己可以选择自己
			return true;
		}
	...
	{
		//由于需求不用显示文字，会干扰CComboUI之下的RichEdit控件，所以此处直接返回
		return;
