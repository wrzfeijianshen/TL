...
void CImageDialog::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Radio(pDX, IDC_RADIO_IMAGE_ALIGN_FALSE, m_bAlignCenter);
...
BEGIN_MESSAGE_MAP(CImageDialog, CDialog)
	...
	ON_BN_CLICKED(IDC_RADIO_IMAGE_ALIGN_FALSE, &CImageDialog::OnBnClickedRadioImageAlignCenterFalse)
	ON_BN_CLICKED(IDC_RADIO_IMAGE_ALIGN_TRUE, &CImageDialog::OnBnClickedRadioImageAlignCenterTrue)
	...
	while (*pStrImage != _T('\0')) {
		...
		if (!sValue.IsEmpty()) {
			if (sItem == _T("file"))
				...
				m_bAlignCenter = (_tcscmp(sValue.GetData(), _T("true")) == 0);
			}
		}
		if (*pStrImage++ != _T(' ')) break;
	}
...
void CImageDialog::SetImageProperty(LPCTSTR pstrPathName)
{
	...
	m_ImagePreview.SetImageProperty(pstrPathName, m_rcDest, m_rcSource, m_rcCorner, clrMask, m_nFade, m_bHole, m_bAlignCenter);
}
...
void CImageDialog::OnBnClickedRadioImageAlignCenterTrue()
{
	// TODO: Add your control notification handler code here
	UpdateData(TRUE);
	SetImageProperty(m_strImagePathName);
...
void CImageDialog::OnBnClickedRadioImageAlignCenterFalse()
{
	// TODO: Add your control notification handler code here
	UpdateData(TRUE);
	SetImageProperty(m_strImagePathName);
