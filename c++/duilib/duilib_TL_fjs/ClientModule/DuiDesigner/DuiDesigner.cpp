...
BOOL CUIDesignerApp::InitInstance()
{
	...
	if (TRUE)
	{
		if (true || (m_lpCmdLine[0] == _T('/0')) || (_tcscmp(m_lpCmdLine, _T("110")) == 0))
			m_bCmdRet = TRUE;
		else
			m_bCmdRet = FALSE;
		...
		if (FALSE == m_bCmdRet)
		{
			...
			MessageBox(hWnd, _T("启动参数错误"), _T("Error"), MB_OK);
			EndDialog(hWnd, 0);
			return FALSE;
		}
	}
...
BOOL CAboutDlg::OnInitDialog()
{
	...
	m_btnMyBlogURL.SetURL(_T("http://blog.csdn.net/dominating_/article/details/51593761"));
	m_btnMyBlogURL.SetTooltip(_T("Aries的博客"));
