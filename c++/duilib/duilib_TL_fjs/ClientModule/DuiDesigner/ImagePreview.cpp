...
void CImagePreview::SetImageProperty(LPCTSTR pstrImage, CRect& rcDest, CRect& rcSource, CRect& rcCorner, COLORREF clrMask, int nFade, BOOL bHole, BOOL bAlignCenter)
{
	...
	if (!m_strImage.IsEmpty())
	{
		...
		if (bAlignCenter == TRUE)
		{
			str = _T(" aligncenter='true'");
			m_strProperty += str;
			bFlag = TRUE;
		}
		if (bFlag)
