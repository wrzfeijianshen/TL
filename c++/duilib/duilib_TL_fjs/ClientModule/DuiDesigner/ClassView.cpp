...
void CClassView::SelectUITreeItem(CControlUI* pControl)
{
	...
	if (pControl->GetTag() > 0)
	{
		...
		if (NULL != m_hItemPrev)
			m_wndClassView.SetItemState(m_hItemPrev, 0, TVIS_BOLD | TVIS_SELECTED);
		
		// 设置这次选中的Item为突出显示  
		m_hItemCur = (HTREEITEM)(((ExtendedAttributes*)pControl->GetTag())->hItem);
		if (NULL != m_hItemCur)
		{
			m_wndClassView.SelectItem(m_hItemCur);
			m_wndClassView.SetItemState(m_hItemCur, TVIS_BOLD | TVIS_SELECTED, TVIS_BOLD | TVIS_SELECTED);
		}

		m_hItemPrev = m_hItemCur;
	}
}
