...
void CPropertiesWnd::SetUIValue(CMFCPropertyGridProperty* pProp, int nTag)
{
	...
	//如果是设置与图片相关的路径，则过滤，取相对路径
	if (true)
	{
		const char* str = StringConvertor::WideToUtf8(strName);
		const std::regex pattern(".+(image)");
		bool bIsImage = std::regex_match(str, pattern);
		if (bIsImage)
		{
			CDuiString sTempPath;
			LPCTSTR lpctStr = strNewVal.GetBuffer();
			strNewVal.ReleaseBuffer();
			if (TRUE == CLayoutManager::ConvertSurplusFileName(sTempPath, lpctStr, FALSE))
			{
				strNewVal = lpctStr;
				strNewVal.Replace(_T("\\"), _T("/"));
				pProp->SetValue((_variant_t)strNewVal);
			}
		}
	}
	...
	if (pRoot != NULL)
		pRoot->SetPos(pRoot->GetPos());

	//更新CUIDesignerView主视图
	Msg.sType = _T("InvalidateView");
	Msg.lParam = NULL;
	pUIView->Notify(Msg);
}