...
void CUIDesignerView::Notify(TNotifyUI& msg)
{
	...
	else if (msg.sType == _T("InvalidateView"))
	{
		this->Invalidate();
	}
...
void CUIDesignerView::ReloadXmlUI()
{
	CUIDesignerDoc* pDoc = GetDocument();
	CFormUI* pForm = m_LayoutManager.GetForm();

	//移除左侧旧的控件树
	g_pClassView->RemoveUITreeItem(pForm);
	g_pResourceView->RemoveImageTree(pDoc->GetTitle());

	//清理CForm下的根节点控件、以及对应标签等
	CControlUI* pFileRoot = pForm->GetFileRoot();
	if (pFileRoot)
	{
		DeleteUI(pFileRoot);
		pForm->SetFileRoot(nullptr);
	}

	//重新加载新xml UI文件
	m_LayoutManager.Reload(pDoc->GetPathName());
...
void CUIDesignerView::OnFormEditTest()
{
	...
	if (_tcscmp(strFilePath, _T("")) == 0)
		strFilePath = CPaintManagerUI::GetResourcePath();
